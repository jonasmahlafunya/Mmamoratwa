...
const _DEFAULT_MWAU_DATA = {
  ...
  staff: [
    { id: 'STF001', name: 'Sipho Dlamini', role: 'Funeral Director', phone: '083 111 2222', available: true },
    { id: 'STF002', name: 'Nomsa Khumalo', role: 'Embalmer', phone: '082 333 4444', available: true },
    { id: 'STF003', name: 'Bongani Nkosi', role: 'Driver', phone: '071 555 6666', available: true },
    { id: 'STF004', name: 'Lerato Molefe', role: 'Grief Counsellor', phone: '073 777 8888', available: true },
    { id: 'STF005', name: 'Thandi Sithole', role: 'Catering Coordinator', phone: '079 999 0000', available: false }
  ],

  vehicles: [
    { id: 'VEH001', name: 'Hearse – GP 12 MWAU', type: 'hearse', status: 'available' },
    { id: 'VEH002', name: 'Family Car 1 – GP 13 MWAU', type: 'family', status: 'available' },
    { id: 'VEH003', name: 'Family Car 2 – GP 14 MWAU', type: 'family', status: 'available' },
    { id: 'VEH004', name: 'Support Van – GP 15 MWAU', type: 'support', status: 'in_use' }
  ],
...
const Auth = {
  ...
  // Returns '../' if currently inside a subdirectory (pages/), else ''
  _base() {
    const depth = window.location.pathname.split('/').filter(Boolean).length;
    // Under file:// each path segment counts; if the file is in /pages/ depth >= 2
    // We just check if the current filename is NOT index.html at root level
    const inSubdir = !window.location.pathname.endsWith('/index.html') &&
      window.location.pathname.split('/').slice(0, -1).join('/').match(/\/pages$/);
    return inSubdir ? '../' : '';
  },
  ...
  requireAuth(role) {
    ...
    if (!session) { window.location.href = base + 'pages/login.html'; return null; }
    if (role && session.role !== role) {
      if (session.role === 'admin') window.location.href = base + 'pages/admin-dashboard.html';
      else window.location.href = base + 'pages/member-dashboard.html';
      return null;
    }
    return session;
  },
...
function initTabs(containerId) {
  ...
  container.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Support data-tab-group, .tabs wrapper, or .tab-nav wrapper
      const tabGroup = btn.closest('[data-tab-group]')
        || btn.closest('.tabs')
        || btn.closest('.tab-nav')?.parentElement
        || btn.parentElement;
      if (!tabGroup) return;
